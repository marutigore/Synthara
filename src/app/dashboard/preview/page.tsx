...
import { Download, UploadCloud, FileText, Settings2, Filter, ChevronsUpDown, Info, Eye, Loader2, DatabaseZap, CheckCircle, Sparkles, X } from "lucide-react";
import Link from 'next/link';
import { getUserDatasets, type SavedDataset, getDatasetById, updateDataset } from '@/lib/supabase/actions';
...
function DataPreviewContent() {
  ...
  const handleCellSave = async (rowIndex: number, colName: string) => {
    if (!editingCell || !loadedDataset) return;
    
    // Update local state
    const updatedRows = [...loadedDataRows];
    updatedRows[rowIndex] = { ...updatedRows[rowIndex], [colName]: editValue };
    setLoadedDataRows(updatedRows);
    setEditingCell(null);

    // Convert back to CSV and save to Supabase
    try {
      const updatedCsv = Papa.unparse(updatedRows);
      const res = await updateDataset(loadedDataset.id, updatedCsv, loadedSchema, updatedRows.length);
      if (res.success) {
        toast({ title: "Saved", description: "Cell changes saved successfully." });
        setLoadedDataset(prev => prev ? { ...prev, data_csv: updatedCsv, num_rows: updatedRows.length } : null);
      } else {
  ...
  const handleAiRefinement = async () => {
    if (!loadedDataset || !aiPrompt) return;
    setIsRefining(true);
    try {
      const geminiKey = localStorage.getItem('synthara_gemini_key') || '';
      
      const response = await fetch('/api/preview/refine', {
        method: 'POST',
      ...
      setLoadedDataRows(result.rows);
      setLoadedSchema(result.schema);
      setLoadedDataset(prev => prev ? { ...prev, data_csv: result.csv, num_rows: result.numRows, schema_json: result.schema } : null);
      setAiPrompt('');
      toast({ title: "Refinement Complete", description: `Successfully refined ${result.numRows} rows.` });
    } catch (err: any) {
  ...
  return (
    <div className="flex w-full gap-8 relative">
      <div className={`flex-1 space-y-10 w-full max-w-[1600px] mx-auto pt-4 pb-12 transition-all duration-300 ${isSidebarOpen ? 'mr-96' : ''}`}>
        {/* Header Section */}
        <div className="glass-modern p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden group">
          <div className="space-y-2 relative z-10 flex-1">
            ...
            <p className="text-sm text-muted-foreground font-medium max-w-2xl leading-relaxed">
              Inspect your saved datasets, review schemas, and download records for external use. Double-click any cell to edit it manually.
            </p>
          </div>
                            ...
                            {loadedDataRows.slice(0, 20).map((row: Record<string, any>, rowIndex: number) => (
                              <TableRow key={rowIndex} className="border-border/10 hover:bg-primary/[0.02] transition-colors group">
                                {loadedSchema.map((col: { name: string, type: string }) => {
                                  ...
                                  const isEditing = editingCell && editingCell.rowIndex === rowIndex && editingCell.colName === col.name;

                                  return (
                                    <TableCell
                                      key={col.name}
                                      className="px-6 py-4 text-sm font-bold text-muted-foreground group-hover:text-foreground transition-colors max-w-[300px] truncate cursor-pointer select-none"
                                    ...
                                    >
                                      {isEditing ? (
                                        <Input
                                          ...
                                          onKeyDown={(e) => {
                                            if (e.key === 'Enter') handleCellSave(rowIndex, col.name);
                                            if (e.key === 'Escape') setEditingCell(null);
                                          }}
                                          autoFocus
                                          className="h-8 rounded-lg bg-background border-primary focus-visible:ring-primary text-foreground text-xs"
                                        />
                                      ) : (
      ...
      {isSidebarOpen && loadedDataset && (
        <div className="w-96 border-l border-border bg-card shadow-2xl p-6 space-y-6 overflow-y-auto z-40 animate-in slide-in-from-right duration-300">
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder="e.g. 'convert email column to lowercase', 'round the pricing column to two decimals', 'remove rows without description'"
                rows={5}
                className="w-full mt-2 p-3 bg-secondary/30 border border-border/50 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-primary/30 text-foreground"
              />
            </div>
            ...
            <Button
              ...
              className="w-full h-12 font-bold bg-primary text-primary-foreground shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-2"
            >
              {isRefining ? (
                <>
          ...
          <div className="p-4 rounded-xl bg-secondary/20 border border-border/30 text-xs text-muted-foreground leading-relaxed space-y-2">
            <p className="font-bold text-foreground">💡 How it works:</p>
            <p>1. The instruction is sent directly to Gemini with your data.</p>
            <p>2. The AI cleans, transforms, or enriches your dataset rows.</p>
            <p>3. If your prompt implies changes to structure, the column schema is automatically updated.</p>
          </div>
        </div>
      )}
    </div>
